          ...
          <div class="col-lg-8 mb-5 p-0">
            <div class="card p-3 shadow-sm">
              <h1 class="font-weight-bold d-flex align-items-center">
                Cari Postingan <span class="ml-1"><small>🔍</small></span>
              </h1>
              <div class="d-flex mt-3 mb-3">
                <input v-model="q" class="form-control shadow-none" />
                <button @click="cari" class="btn btn-primary ml-3 shadow-none">
                  Cari
                </button>
              </div>
            </div>
            ...
            <div
              v-for="p in posts"
              v-if="p.title.toLowerCase().search(q.toLowerCase()) !== -1"
              :key="p.title"
...
export default {
  ...
  methods: {
    async cari() {
      let posts = await this.$content("posts")
