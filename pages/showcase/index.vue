                ...
                <div class='col-lg-8 p-0 mb-5'>
                    ...
                    <div class='card shadow-sm p-3 mt-5'>
                        <div class='d-block d-md-flex justify-content-between'>
                            <div>
                                <h3 class=''><span class='font-weight-bold'>Showcase Terbaru</span></h3>
                            </div>
                            <div class='mt-3 mt-md-0 d-flex'>
                                <input v-model='q' class='form-control shadow-none h-100' placeholder='Cari Project...'>
                        ...
                        <div v-for='s in project' v-if='s.project.toLowerCase().search(q.toLowerCase()) !== -1' :key='s.project' class='col-md-6 col-lg-6 mb-5'>
                            <div class='card p-3 shadow-sm h-100'>
                                ...
                                <div class='card-body p-0 mt-3'>
                                    ...
                                    <hr class='border'>
                                    <div class='mb-2'>
                                        <div :class='"badge-"+gachaWarna()' v-for='t in s.tag' class='badge mr-2'>{{ t }}</div>
                                    </div>
                                    ...
                                    <div class='d-flex justify-content-between'>
                                        <div v-if='s.url'>
                                            <a :href="s.url" target="_blank">
                                                <button class="btn shadow-sm card text-primary">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" viewBox="0 0 16 16" class="bi bi-globe mb-1">
                                        ...
                                        <div v-if='s.download'>
                                            <a :href="s.download" target="_blank">
                                                <button class="btn shadow-sm card text-primary">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" viewBox="0 0 16 16" class="bi bi-download mb-1">
                                        ...
                                        <div v-if='s.repository'>
                                            <a :href="'https://github.com/'+s.username+'/'+s.repository" target="_blank">
                                                <button class="btn shadow-sm card text-primary">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" viewBox="0 0 16 16" class="bi bi-github mb-1">
                    ...
                    <infinite-loading spinner='waveDots' v-if='project.length > 0' class='mt-5 pt-3' @infinite="infiniteScroll">
...
export default {
    async asyncData({
        ...
        const project = await $content('showcase').limit(7).sortBy('createdAt', 'desc').fetch()
        const n = 1;
        let q = '';
        return {
            project,
    ...
    methods: {
        async cari() {
            let project = await this.$content('showcase').limit(7).sortBy('createdAt', 'desc').fetch();
        ...
        async infiniteScroll($state) {
            let dataBaru = await this.$content('showcase').skip(7 * this.n).limit(7).sortBy('createdAt', 'desc').fetch();
            this.n++;
